...
fig1 = bkp.figure(width=800, match_aspect = True)
...
fig1.x_range = Range1d(start = -3.0, end = 14.0)
fig1.y_range = Range1d(start = -2.0, end = 12.0)
...
fig1.line('x_vec','y_vec',source =data_slot,  line_width = 2.0, line_color = 'black', line_dash = 'solid')
...
fig1.multi_line('x_vec','y_vec',source =data_other_slot,  line_width = 2.0, line_color = 'black', line_dash = 'solid')
...
fig1.patch('x_vec', 'y_vec', source = data_tra_tb_pt, fill_color='blue', fill_alpha=0.5, line_color=None, legend_label='Traditional boundary')

fig1.patches("x_vec", "y_vec", source=data_extra_region, color='green', fill_alpha=1, line_color=None, legend_label='Extra available space')
fig1.patches("x_vec", "y_vec", source=data_obs_car_polygon, color='white', fill_alpha=1, line_color=None)

# obstacles
fig1.scatter("x_vec", "y_vec", source=data_fus_obs, size=3, color='grey')
fig1.scatter("x_vec", "y_vec", source=data_obs_pt, size=3, color='red')
...
fig1.patch( 'car_xn', 'car_yn', source = data_start_car, fill_color = "red", fill_alpha = 0.6, line_color = "black", line_width = 2)
...
# fig1.scatter("x_vec", "y_vec", source=data_virtual_obs_pt, size=8, color='red', marker='star')
...
fig1.legend.label_text_font_size = '16pt'  # 设置图例字体大小
...
class LocalViewSlider:
  def __init__(self,  slider_callback):
    ...
    self.ego_heading_slider = ipywidgets.FloatSlider(layout=ipywidgets.Layout(width='75%'), description= "ego_heading",min=-180, max=180, value= -0.1, step=0.1)
    # slot pt
...
export_svgs(fig1, filename=file_name + "svg")

svg_file_path = file_name + "svg"
eps_file_path = file_name + "eps"
...
cairosvg.svg2pdf(url=svg_file_path, write_to=file_name + "pdf")
